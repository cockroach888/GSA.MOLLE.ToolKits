...
namespace GSA.ToolKits.DBUtility
{
    ...
    public sealed class SQLiteHelper
    {
        ...
        public static DataTable SelectPaging(string connectionStringing, string tableName, string strColumns, string strWhere, string strOrder, int pageSize, int currentIndex, out int recordOut)
        {
            DataTable dt = new DataTable();
            string cmdSql = string.Format("select count(*) from {0} where {1}", tableName, strWhere);
            recordOut = Convert.ToInt32(ExecuteScalar(connectionStringing, CommandType.Text, cmdSql));
            cmdSql = "select {0} from {1} where {2} order by {3} limit {4} offset {5}";
            int offsetCount = (currentIndex - 1) * pageSize;
            cmdSql = string.Format(cmdSql, strColumns, tableName, strWhere, strOrder, pageSize.ToString(), offsetCount.ToString());
            using (SQLiteDataReader reader = ExecuteReader(connectionStringing, CommandType.Text, cmdSql))
            {
                if (reader != null)
                {
                    dt.Load(reader);
