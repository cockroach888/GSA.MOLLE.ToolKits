...
**本库提供用于处理各种图像的辅助与应用需求的相关工具，诸如图像与Base64相互转换、图像落盘存储等，支持多种版本（net8.0;net9.0;net10.0;）。**
...
### 2025-10-18
- 更新 SixLabors.ImageSharp 依赖到最新版本；
- 优化图像从Base64编码字符串解码为字节数组的编码实现；
- 增加获取图像宽度和高度返回元组的方法；
- 增加将Base64编码字符串解码为内存流的编码实现；
- 增加根据图片文件扩展名获取图像MimeType枚举编码实现。
